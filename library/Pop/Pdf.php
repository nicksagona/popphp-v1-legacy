...
class Pop_Pdf extends Pop_File
{
    ...
    public function __construct($pdf, $sz = null, $w = null, $h = null)
    {
        ...
        $this->_fillColor = new Pop_Color_Rgb(0, 0, 0);
        $this->_backgroundColor = new Pop_Color_Rgb(255, 255, 255);

        parent::__construct($pdf);

        $this->_objects[1] = new Pop_Pdf_Root();
        $this->_objects[2] = new Pop_Pdf_Parent();
        $this->_objects[3] = new Pop_Pdf_Info();
    ...
    public function importPdf($pdf, $pg = null)
    {
        ...
        $pdfi = new Pop_Pdf_Import($pdf, $pg);
        ...
        foreach($importedObjs as $key => $value) {
            if ($value['type'] == 'page') {
                ...
                $this->_objects[$key] = new Pop_Pdf_Object($value['data']);
            }
        }
    ...
    public function addPage($sz = null, $w = null, $h = null)
    {
        ...
        $this->_objects[$pi] = new Pop_Pdf_Page(null, $sz, $w, $h, $pi);
    ...
    public function copyPage($pg)
    {
        ...
        if (!array_key_exists($key, $this->_pages)) {
            ...
            $pi = $this->_lastIndex($this->_objects) + 1;
            $ci = $this->_lastIndex($this->_objects) + 2;
            $this->_objects[$pi] = new Pop_Pdf_Page($this->_objects[$this->_pages[$key]]);
            ...
            foreach ($oldContent as $key => $value) {
                $this->_objects[$ci] = new Pop_Pdf_Object((string)$this->_objects[$value]);
    ...
    public function addFont($font)
    {
        ...
        if (!array_key_exists($font, $this->_standard_fonts)) {
            ...
            $this->_objects[$i] = new Pop_Pdf_Object("{$i} 0 obj\n<<\n    /Type /Font\n    /Subtype /Type1\n    /Name /{$f}\n    /BaseFont /{$font}\n    /Encoding /StandardEncoding\n>>\nendobj\n\n");
    ...
    public function embedFont($file)
    {
        ...
        $fontParser = new Pop_Pdf_Parser_Font($file, $fontIndex, $objectIndex, $this->_compress);
    ...
    public function addImage($image, $x, $y, $scl = null)
    {
        ...
        $i = $this->_lastIndex($this->_objects) + 1;
        $imageParser = new Pop_Pdf_Parser_Image($image, $x, $y, $i, $scl);
    ...
    public function finalize()
    {
        ...
        foreach ($this->_objects as $obj) {
            if ($obj->index != $this->_root) {
                if (($obj instanceof Pop_Pdf_Object) && ($this->_compress) && (!$obj->isPalette()) && (!$obj->isCompressed())) {
                    $obj->compress();
                }
    ...
    protected function _getContentObject()
    {
        ...
        if (null === $this->_objects[$this->_objects[$this->_pages[$this->_curPage]]->index]->curContent) {
            ...
            $this->_objects[$coi] = new Pop_Pdf_Object($coi);
