...
class Pop_Locale
{
     ...
     * @return void
     */
    public static function load($lng = null)
    {
        return new self($lng);
    }
        ...
        if (!array_key_exists($lang, $langCodes)) {
            ...
            if (strpos($trans, 'Bad Request') !== false) {
                ...
                for ($i = 0; $i < count($langText); $i++) {
                    $text = new Pop_Dom_Child('text', null, null, false, '    ');
                    $source = new Pop_Dom_Child('source', str_replace(' & ', ' &amp; ', $langText[$i]), null, false, '        ');
                    $output = new Pop_Dom_Child('output', str_replace(' & ', ' &amp; ', $transText[$i]), null, false, '        ');
                ...
                // Else, output to file.
                    ...
                    $langXMLFile = new Pop_File($dest . '/' . $lang . '.xml');
    ...
    {
        $this->loadFile(dirname(__FILE__) . '/Locale/Data/' . $this->_language . '.xml');
    }
