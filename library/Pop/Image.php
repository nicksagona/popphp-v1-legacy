...
class Pop_Image extends Pop_Image_Abstract
{
    ...
    public function __construct($img, $w = null, $h = null, Pop_Color_Interface $color = null, $types = null)
    {
        ...
        if (!self::isGdInstalled()) {
            throw new Exception($this->_lang->__('Error: The GD library extension must be installed to use the Pop_Image component.'));
        } else {
            $this->_getGdInfo();
     ...
     * @return Pop_Image
     */
    public function blur($amount, $type = Pop_Image::GAUSSIAN_BLUR)
    {
        // Create an image resource.
        $this->_createResource();
        $blurType = ($type == Pop_Image::GAUSSIAN_BLUR) ? IMG_FILTER_GAUSSIAN_BLUR : IMG_FILTER_SELECTIVE_BLUR;
    ...
    public function border($w, $h = null, $type = Pop_Image::INNER_BORDER)
    {
        ...
        if ($type == Pop_Image::INNER_BORDER) {
