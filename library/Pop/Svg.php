...
class Pop_Svg extends Pop_File
{
    ...
    public function __construct($svg, $w = null, $h = null, Pop_Color_Interface $color = null)
    {
        ...
        // properties of the new SVG image have been passed.
            ...
            if (is_null($w) || is_null($h)) {
                ...
                $this->_backgroundColor = (!is_null($color)) ? $color : new Pop_Color_Rgb(255, 255, 255);
                ...
                if (!is_null($color)) {
                    ...
                    $rect->addAttribute('fill', $color->getRgb(Pop_Color::STRING, true));
                }
            }
        }
    ...
    {
        if (is_null($wid) || ($wid == false) || ($wid == 0)) {
     ...
     * @return Pop_Svg
     */
    public function addGradient(Pop_Color_Interface $color1, Pop_Color_Interface $color2, $type = Pop_Svg::HORIZONTAL)
    {
        ...
        $stop1->addAttribute('style', 'stop-color: ' . $color1->getRgb(Pop_Color::STRING, true) . '; stop-opacity: 1;');

        $stop2 = $grad->addChild('stop');
        $stop2->addAttribute('offset', '100%');
        $stop2->addAttribute('style', 'stop-color: ' . $color2->getRgb(Pop_Color::STRING, true) . '; stop-opacity: 1;');
    ...
    {
        if (!is_null($index) && array_key_exists($index, $this->_gradients)) {
    ...
    public function addClippingRectangle($x, $y, $w, $h = null)
    {
        ...
        $rect->addAttribute('height', (is_null($h) ? $w : $h) . $this->_units);
    ...
    public function addClippingEllipse($x, $y, $w, $h = null)
    {
        ...
        $ellipse->addAttribute('ry', (is_null($h) ? $w : $h) . $this->_units);
    ...
    {
        if (!is_null($index) && array_key_exists($index, $this->_clippingPaths)) {
    ...
    public function text($str, $size, $x, $y, $font = 'Arial', $rotate = null, $bold = false)
    {
        ...
        if (!is_null($this->_fillColor)) {
            $text->addAttribute('fill', $this->_fillColor->getRgb(Pop_Color::STRING, true));
    ...
    public function border($w)
    {
        ...
        $color = (!is_null($this->_strokeColor)) ? $this->_strokeColor : new Pop_Color_Rgb(0, 0, 0);

        $rect->addAttribute('stroke', $color->getRgb(Pop_Color::STRING, true));
        $rect->addAttribute('stroke-width', ($w * 2) . $this->_units);
        if (!is_null($this->_strokeDashLength) && !is_null($this->_strokeDashGap)) {
    ...
    {
        if (!is_null($this->_curClippingPath)) {
            $obj->addAttribute('style', 'clip-path: url(#clip' . $this->_curClippingPath .');');
        }

        if (!is_null($this->_curGradient)) {
            $obj->addAttribute('fill', 'url(#grad' . $this->_curGradient . ')');
        } else if (!is_null($this->_fillColor)) {
            $obj->addAttribute('fill', $this->_fillColor->getRgb(Pop_Color::STRING, true));
            if ($this->_opacity < 1.0) {
                $obj->addAttribute('fill-opacity', $this->_opacity);
            }
        }
        if (!is_null($this->_strokeColor)) {
            $obj->addAttribute('stroke', $this->_strokeColor->getRgb(Pop_Color::STRING, true));
            $obj->addAttribute('stroke-width', (!is_null($this->_strokeWidth) ? $this->_strokeWidth : 1) . $this->_units);
            if (!is_null($this->_strokeDashLength) && !is_null($this->_strokeDashGap)) {
