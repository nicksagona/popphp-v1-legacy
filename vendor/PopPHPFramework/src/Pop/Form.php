...
class Pop_Form extends Pop_Dom
{
    ...
    public function __construct($action, $method, $indent = null)
    {
        ...
        $this->_form = new Pop_Dom_Child('form', null, null, false, $indent);
    ...
    {
        if (!is_array($values)) {
            throw new Exception(Pop_Locale::load()->__('The parameter passed must be an array.'));
        } if (isset($values[0]) && !is_array($values[0])) {
            throw new Exception(Pop_Locale::load()->__('The array parameter passed must contain an array.'));
        } else {
        ...
        if (isset($this->_initValues[0])) {
            foreach ($this->_initValues as $field) {
                if (is_array($field) && isset($field['type']) && isset($field['name'])) {
                    ...
                    switch ($type) {
                        case 'checkbox':
                            $elem = new Pop_Form_Element_Checkbox($name, $value, $marked);
                            break;
                        case 'radio':
                            $elem = new Pop_Form_Element_Radio($name, $value, $marked);
                            break;
                        case 'select':
                            $elem = new Pop_Form_Element_Select($name, $value, $marked);
                            break;
                        case 'textarea':
                            $elem = new Pop_Form_Element_Textarea($name, $value, $marked);
                            break;
                        default:
                            $elem = new Pop_Form_Element($type, $name, $value, $marked);
                    }
            ...
            foreach ($this->_initValues as $key => $field) {
                if (isset($field['name']) && isset($values[$field['name']])) {
                    if (($field['type'] == 'select') || ($field['type'] == 'checkbox') || ($field['type'] == 'radio')) {
                        ...
                        $this->_initValues[$key]['value'] = ($filter) ? (string)Pop_String::factory($values[$field['name']])->striptags() : $values[$field['name']];
                    }
                }
            }
        // Else, if elements have already been created, set field values into those elements.
            ...
            foreach ($elements as $key => $field) {
                ...
                if (isset($attributes['name']) && isset($values[$attributes['name']])) {
                    ...
                    } else if ($field instanceof Pop_Form_Element_Textarea) {
                        $val = ($filter) ? (string)Pop_String::factory($values[$attributes['name']])->striptags() : $values[$attributes['name']];
                ...
                } else if ($field instanceof Pop_Form_Element_Checkbox) {
                ...
                // If a radio element.
                } else if ($field instanceof Pop_Form_Element_Radio) {
                    $children = $field->getChildren();
                    $atts = $children[0]->getAttributes();
                    $name = $atts['name'];
                    if (isset($values[$name])) {
    ...
    public function addElements($e)
    {
        ...
        foreach ($children as $child) {
            ...
            if ($child instanceof Pop_Form_Element_Textarea) {
                if (isset($attribs['name'])) {
                    $this->_fields[$attribs['name']] = (isset($child->value) ? $child->value : null);
                }
            } else if ($child instanceof Pop_Form_Element_Select) {
                if (isset($attribs['name'])) {
                    $this->_fields[$attribs['name']] = (isset($child->marked) ? $child->marked : null);
                }
            } else if ($child instanceof Pop_Form_Element_Radio) {
            ...
            } else if ($child instanceof Pop_Form_Element_Checkbox) {
                $radioChildren = $child->getChildren();
                $childAttribs = $radioChildren[0]->getAttributes();
                if (isset($childAttribs['name'])) {
    ...
    public function __toString()
    {
        ...
        // Create DL element.
        $dl = new Pop_Dom_Child('dl', null, null, false, ($this->_form->getIndent() . '    '));
        ...
        foreach ($children as $child) {
            ...
            if (null !== $child->label) {
                ...
                $label = new Pop_Dom_Child('label', $child->label, null, false, ($this->_form->getIndent() . '            '));
