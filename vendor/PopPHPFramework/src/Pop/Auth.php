...
class Pop_Auth
{
    ...
    {
        if (!is_array($options)) {
            throw new Exception(Pop_Locale::load()->__('The options parameter must be an array.'));
        } else if (!isset($options['table']) && !isset($options['file'])) {
            throw new Exception(Pop_Locale::load()->__('The options parameter must be an array that contains either a \'table\' or \'file\' key.'));
        } else {
