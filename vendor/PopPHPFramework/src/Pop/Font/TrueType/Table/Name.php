...
class Pop_Font_TrueType_Table_Name
{
     ...
     * @var array
     */
    protected $_names = array(0  => 'copyright',
                              1  => 'fontFamily',
                              2  => 'fontSubFamily',
                              3  => 'uniqueId',
                              4  => 'fullName',
    ...
    {
        $font->tableInfo['name']->header = new ArrayObject(unpack('nformatSelector/' .
                                                                  'nnameRecordsCount/' .
                                                                  'nstorageOffset', $font->read($font->tableInfo['name']->offset, 6)), ArrayObject::ARRAY_AS_PROPS);

        $bytePos = $font->tableInfo['name']->offset + 6;

        for ($j = 0; $j < $font->tableInfo['name']->header->nameRecordsCount; $j++) {
            $ttfRecord = unpack('nplatformId/' .
                                'nencodingId/' .
                                'nlanguageId/' .
                                'nnameId/' .
                                'nlength/' .
